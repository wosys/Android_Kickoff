...
package lib.wintmain.permissionbase
...
/**
 * A variation of [PermissionsBox] that takes a list of permissions and only calls [onGranted] when
 * all the [requiredPermissions] are granted.
 *
 * By default it assumes that all [permissions] are required.
 */
@OptIn(ExperimentalPermissionsApi::class) // Accompanist Permissions is experimental. The API may be changed in the future.
@Composable
fun PermissionsBox(
    modifier: Modifier = Modifier,
    ...
    val permissionState = rememberMultiplePermissionsState(permissions = permissions) { map ->
        ...
        errorText = if (rejectedPermissions.none { it in requiredPermissions }) {
            ...
            "Need ${rejectedPermissions.joinToString()} permission(s) to continue."
        }
    }
