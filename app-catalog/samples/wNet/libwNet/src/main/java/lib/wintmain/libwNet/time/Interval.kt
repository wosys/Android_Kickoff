...
package lib.wintmain.libwNet.time
...
import lib.wintmain.libwNet.time.IntervalStatus.STATE_ACTIVE
import lib.wintmain.libwNet.time.IntervalStatus.STATE_IDLE
import lib.wintmain.libwNet.time.IntervalStatus.STATE_PAUSE
...
open class Interval @JvmOverloads constructor(
     ...
     */
    fun start() = apply {
        if (state == STATE_ACTIVE) return this
        state = STATE_ACTIVE
        count = start
        launch()
    }
