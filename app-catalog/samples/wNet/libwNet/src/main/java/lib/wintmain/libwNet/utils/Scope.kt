...
import lib.wintmain.libwNet.scope.DialogCoroutineScope
import lib.wintmain.libwNet.scope.NetCoroutineScope
import lib.wintmain.libwNet.scope.PageCoroutineScope
import lib.wintmain.libwNet.scope.StateCoroutineScope
import lib.wintmain.libwNet.scope.ViewCoroutineScope
 ...
 * 可以设置全局对话框 [lib.wintmain.libwNet.NetConfig.dialogFactory]
 * 对话框被取消或者界面关闭作用域被取消
 *
 ...
 * 作用域正常结束时显示成功缺省页
 * 作用域抛出异常时显示错误缺省页
 * 并且自动吐司错误信息, 可配置 [lib.wintmain.libwNet.interfaces.NetErrorHandler.onStateError]
 * 自动打印异常日志
 * 布局被销毁或者界面关闭作用域被取消
 ...
 * 3. 捕获异常
 * 4. 打印异常日志
 * 5. 吐司部分异常[lib.wintmain.libwNet.interfaces.NetErrorHandler.onStateError]
 * 6. 判断添加还是覆盖数据
 * 7. 自动显示缺省页
 *
 * 布局被销毁或者界面关闭作用域被取消
...
/**
 * 该函数比[scope]多了以下功能
 * - 在作用域内抛出异常时会被回调的[lib.wintmain.libwNet.interfaces.NetErrorHandler.onError]函数中
 * - 自动显示错误信息吐司, 可以通过指定[lib.wintmain.libwNet.interfaces.NetErrorHandler.onError]来取消或者增加自己的处理
 *
 * 该作用域生命周期跟随整个应用, 注意内存泄漏
 * @param dispatcher 调度器, 默认运行在[Dispatchers.Main]即主线程下
 */
