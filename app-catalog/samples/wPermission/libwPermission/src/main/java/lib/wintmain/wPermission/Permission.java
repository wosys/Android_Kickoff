...
public final class Permission {
     ...
     * 2.0.1 实测不行 荣耀：Magic UI 6.0 及以上版本，Magic UI 5.0 实测不行 小米：Miui 13 及以上版本，Miui 12 实测不行，经过验证 miui
     * 上面默认会授予此权限 OPPO：(ColorOs 12 及以上版本 && Android 11+) || (ColorOs 11.1 及以上版本 && Android 12+)
     * VIVO：虽然没有申请这个权限的通道，但是读取已安装第三方应用列表是没有问题的，没有任何限制 真我：realme UI 3.0 及以上版本，realme UI 2.0 实测不行
     */
     ...
     *
     * <p>为了兼容 Android 11 以下版本，需要在清单文件中注册 {@link Permission#READ_EXTERNAL_STORAGE} 和 {@link
     * Permission#WRITE_EXTERNAL_STORAGE} 权限
     *
     * <p>如果你的应用需要上架 GooglePlay，那么需要详细阅读谷歌应用商店的政策：
     ...
     *
     * <p>Android 11 特性调整，安装外部来源应用需要重启 App：https://cloud.tencent.com/developer/news/637591
     ...
     *
     * <p>为了兼容 Android 13 以下版本，框架会自动申请 {@link #NOTIFICATION_SERVICE} 权限
     */
     ...
     *
     * <p>需要在清单文件中加入 android:usesPermissionFlags="neverForLocation" 属性（表示不推导设备地理位置）
     * 否则就会导致在没有定位权限的情况下扫描不到附近的 WIFI 设备，这个是经过测试的，下面是清单权限注册案例，请参考以下进行注册 <uses-permission
     * android:name="android.permission.NEARBY_WIFI_DEVICES"
     * android:usesPermissionFlags="neverForLocation" tools:targetApi="s" />
     *
     * <p>为了兼容 Android 13 以下版本，需要清单文件中注册 {@link #ACCESS_FINE_LOCATION} 权限 还有 Android 13 以下设备，使用 WIFI
     * 需要精确定位权限，框架会自动在旧的安卓设备上自动添加此权限进行动态申请
     */
    public static final String NEARBY_WIFI_DEVICES = "android.permission.NEARBY_WIFI_DEVICES";
    /**
     * 后台传感器权限（Android 13.0 新增的权限）
     *
     * <p>需要注意的是： 1. 一旦你申请了该权限，在授权的时候，需要选择《始终允许》，而不能选择《仅在使用中允许》 2. 如果你的 App
     * 只在前台状态下使用传感器功能，请不要申请该权限（后台传感器权限）
     */
     ...
     *
     * <p>为了兼容 Android 13 以下版本，需要在清单文件中注册 {@link #READ_EXTERNAL_STORAGE} {@link
     ...
     *
     * <p>需要在清单文件中加入 android:usesPermissionFlags="neverForLocation" 属性（表示不推导设备地理位置）
     * 否则就会导致在没有定位权限的情况下扫描不到附近的蓝牙设备，这个是经过测试的，下面是清单权限注册案例，请参考以下进行注册 <uses-permission
     * android:name="android.permission.BLUETOOTH_SCAN"
     * android:usesPermissionFlags="neverForLocation" tools:targetApi="s" />
     *
     * <p>为了兼容 Android 12 以下版本，需要清单文件中注册 {@link Manifest.permission#BLUETOOTH_ADMIN} 权限 还有 Android
     * 12 以下设备，获取蓝牙扫描结果需要精确定位权限，框架会自动在旧的安卓设备上自动添加此权限进行动态申请
     */
    public static final String BLUETOOTH_SCAN = "android.permission.BLUETOOTH_SCAN";
    /**
     * 蓝牙连接权限（Android 12.0 新增的权限）
     *
     * <p>为了兼容 Android 12 以下版本，需要在清单文件中注册 {@link Manifest.permission#BLUETOOTH} 权限
     */
    public static final String BLUETOOTH_CONNECT = "android.permission.BLUETOOTH_CONNECT";
    /**
     * 蓝牙广播权限（Android 12.0 新增的权限）
     *
     * <p>将当前设备的蓝牙进行广播，供其他设备扫描时需要用到该权限 为了兼容 Android 12 以下版本，需要在清单文件中注册 {@link
     ...
     *
     * <p>需要注意的是： 1. 一旦你申请了该权限，在授权的时候，需要选择《始终允许》，而不能选择《仅在使用中允许》 2. 如果你的 App
     * 只在前台状态下使用定位功能，没有在后台使用的场景，请不要申请该权限
     */
     ...
     *
     * <p>需要注意的是：Android 10 以下不需要传感器（BODY_SENSORS）权限也能获取到步数
     */
    public static final String ACTIVITY_RECOGNITION = "android.permission.ACTIVITY_RECOGNITION";
    /**
     * 读取照片中的地理位置（Android 10.0 新增的权限）
     *
     * <p>需要注意的是：如果这个权限申请成功了但是不能正常读取照片的地理信息，那么需要先申请存储权限，具体可分别下面两种情况：
     *
     * <p>1. 如果适配了分区存储的情况下： 1) 如果项目 targetSdkVersion <= 32 需要申请 {@link
     * Permission#READ_EXTERNAL_STORAGE} 2) 如果项目 targetSdkVersion >= 33 需要申请 {@link
     * Permission#READ_MEDIA_IMAGES}
     *
     * <p>2. 如果没有适配分区存储的情况下： 1) 如果项目 targetSdkVersion <= 29 需要申请 {@link
     * Permission#READ_EXTERNAL_STORAGE} 2) 如果项目 targetSdkVersion >= 30 需要申请 {@link
     ...
     * 读取电话状态
     *
     * <p>需要注意的是：这个权限在某些手机上面是没办法获取到的，因为某些系统禁止应用获得该权限 所以你要是申请了这个权限之后没有弹授权框，而是直接回调授权失败方法 请不要惊慌，这个不是
     * Bug、不是 Bug、不是 Bug，而是正常现象
     *
     * <p>后续情况汇报：有人反馈在 iQOO 手机上面获取不到该权限，在清单文件加入下面这个权限就可以了（这里只是做记录，并不代表这种方式就一定有效果） <uses-permission
